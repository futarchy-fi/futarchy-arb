...
// =============================================================================
// PERMISSIONLESS FLASH ARBITRAGE CONTRACT (V5)
// =============================================================================

/**
 * @title GnosisFlashArbitrageV5
 * @notice PERMISSIONLESS flash loan arbitrage using Balancer V3 Vault on Gnosis Chain
 * @dev V5: Replaces Balancer V2 3-hop GNO<->sDAI swap with direct Swapr/Algebra
 *      spot pool swap (~0.67% fee vs ~3% Balancer V2 3-hop loss).
 *
 * Key differences from V4:
 * - Removed IBalancerV2Vault interface and balancerV2Vault state variable
 * - Added spotPool (GNO/sDAI Algebra pool) for direct swaps
 * - _swapGnoToSdaiV2 and _swapSdaiToGnoV2 replaced with _swapOnPool(spotPool, ...)
 */
contract GnosisFlashArbitrageV5 is ReentrancyGuard {
    ...
    address public immutable spotPool;  // GNO/sDAI Algebra pool for direct swaps

    // Admin for emergency recovery only (not for access control)
    ...
    constructor(
        ...
        spotPool = _spotPool;
        admin = msg.sender;  // Deployer becomes admin for recovery only
    }
     ...
     * @dev Profits are sent to msg.sender (the caller)
    ...
    function executeArbitrage(
        ...
        // Store state for callback - profits go to caller
        _activeProposal = proposalAddress;
        _activeDirection = direction;
        _minProfit = minProfit;
        _profitRecipient = msg.sender;  // Caller receives all profits
    ...
    function onUnlock(
        ...
        // 5. Send profit to caller (NOT owner - fully permissionless)
        if (profit > 0) {
    ...
    function _executeSpotSplit(
        ...
        // 5. Swap sDAI -> GNO via spot pool (direct, ~0.67% fee)
        uint256 sdaiBalance = IERC20(sdaiToken).balanceOf(address(this));
        if (sdaiBalance > 0) {
            _swapOnPool(spotPool, sdaiToken, sdaiBalance);
        }
    }
    ...
    function _executeMergeSpot(
        ...
        // 5. Swap GNO -> sDAI via spot pool (direct, ~0.67% fee)
        uint256 gnoBalance = IERC20(gnoToken).balanceOf(address(this));
        if (gnoBalance > 0) {
            _swapOnPool(spotPool, gnoToken, gnoBalance);
        }
    }
    ...
    /**
     * @notice Transfer admin role (for emergency token recovery)
     * @dev This does NOT affect who can execute arbitrage - that's permissionless
     */
     ...
     * @dev Only admin can call, only for emergency recovery
     */
